...
3. 80-20 split to include peak values in training
...
# --- Shard Requirements Analysis ---

def map_threads_to_shards(thread_count):
    """
    Map thread count to required shards based on the provided chart
    """
    if thread_count <= 6:
        return 1
    elif thread_count <= 8:
        ...
        return 7  # Representing 6+ as 7 for analysis

# Convert thread predictions to shard requirements
actual_shards = np.array([map_threads_to_shards(threads) for threads in y_test])
predicted_shards = np.array([map_threads_to_shards(threads) for threads in y_pred])

# Calculate shard prediction accuracy
shard_matches = (actual_shards == predicted_shards).sum()
shard_accuracy = shard_matches / len(actual_shards)

print(f"\n--- Shard Requirements Analysis ---")
print(f"Total test samples: {len(actual_shards)}")
print(f"Exact shard matches: {shard_matches}")
print(f"Shard prediction accuracy: {shard_accuracy:.3f} ({shard_accuracy*100:.1f}%)")

# Analyze over/under-provisioning in terms of shards
shard_diff = predicted_shards - actual_shards
shard_over_provision = (shard_diff > 0).sum()
shard_under_provision = (shard_diff < 0).sum()
shard_exact = (shard_diff == 0).sum()

print(f"Shard over-provisioning cases: {shard_over_provision} ({shard_over_provision/len(actual_shards)*100:.1f}%)")
print(f"Shard under-provisioning cases: {shard_under_provision} ({shard_under_provision/len(actual_shards)*100:.1f}%)")
print(f"Exact shard matches: {shard_exact} ({shard_exact/len(actual_shards)*100:.1f}%)")

# Detailed shard difference analysis
print(f"Average shard difference: {np.mean(shard_diff):.3f}")
print(f"Median shard difference: {np.median(shard_diff):.3f}")
print(f"Max shard over-provisioning: {np.max(shard_diff)}")
print(f"Max shard under-provisioning: {abs(np.min(shard_diff))}")

# Shard distribution comparison
print(f"\n--- Shard Distribution Comparison ---")
actual_shard_dist = pd.Series(actual_shards).value_counts().sort_index()
predicted_shard_dist = pd.Series(predicted_shards).value_counts().sort_index()

shard_labels = {1: '1 shard (1-6 threads)', 2: '2 shards (7-8 threads)', 
               3: '3 shards (9-10 threads)', 4: '4 shards (11-12 threads)',
               5: '5 shards (13 threads)', 6: '6 shards (14 threads)', 
               7: '6+ shards (15+ threads)'}

print("Actual vs Predicted Shard Distribution:")
for shard_count in sorted(set(actual_shards) | set(predicted_shards)):
    actual_count = actual_shard_dist.get(shard_count, 0)
    predicted_count = predicted_shard_dist.get(shard_count, 0)
    shard_label = shard_labels.get(shard_count, f'{shard_count} shards')
    print(f"  {shard_label}: Actual={actual_count}, Predicted={predicted_count}")

# Plot shard distribution comparison
plt.figure(figsize=(12, 6))
shard_counts = sorted(set(actual_shards) | set(predicted_shards))
actual_counts = [actual_shard_dist.get(shard, 0) for shard in shard_counts]
predicted_counts = [predicted_shard_dist.get(shard, 0) for shard in shard_counts]
shard_names = [shard_labels.get(shard, f'{shard} shards') for shard in shard_counts]

x = np.arange(len(shard_names))
width = 0.35

plt.bar(x - width/2, actual_counts, width, label='Actual', color='lightcoral', alpha=0.8)
plt.bar(x + width/2, predicted_counts, width, label='Predicted', color='lightblue', alpha=0.8)

plt.title('Shard Distribution: Actual vs Predicted', fontsize=14)
plt.xlabel('Shard Requirements')
plt.ylabel('Number of Cases')
plt.xticks(x, shard_names, rotation=45, ha='right')
plt.legend()
plt.grid(True, alpha=0.3)

# Add value labels on bars
for i, (actual, predicted) in enumerate(zip(actual_counts, predicted_counts)):
    plt.text(i - width/2, actual + 1, str(actual), ha='center', va='bottom')
    plt.text(i + width/2, predicted + 1, str(predicted), ha='center', va='bottom')

plt.tight_layout()
plt.show()

# Analysis by thread load levels for shard accuracy
print(f"\n--- Shard Accuracy by Load Level ---")
if low_load_mask.sum() > 0:
    low_shard_accuracy = (actual_shards[low_load_mask] == predicted_shards[low_load_mask]).mean()
    print(f"Low Load (≤5 threads) shard accuracy: {low_shard_accuracy:.3f} ({low_load_mask.sum()} samples)")

if medium_load_mask.sum() > 0:
    medium_shard_accuracy = (actual_shards[medium_load_mask] == predicted_shards[medium_load_mask]).mean()
    print(f"Medium Load (6-15 threads) shard accuracy: {medium_shard_accuracy:.3f} ({medium_load_mask.sum()} samples)")

if high_load_mask.sum() > 0:
    high_shard_accuracy = (actual_shards[high_load_mask] == predicted_shards[high_load_mask]).mean()
    print(f"High Load (>15 threads) shard accuracy: {high_shard_accuracy:.3f} ({high_load_mask.sum()} samples)")

# Critical under-provisioning analysis (cases where we predict fewer shards than needed)
critical_under_provision = shard_diff < 0
if critical_under_provision.sum() > 0:
    print(f"\n--- Critical Under-Provisioning Analysis ---")
    print(f"Cases with insufficient shards: {critical_under_provision.sum()}")
    
    # Show the worst under-provisioning cases
    worst_cases = np.where(shard_diff == np.min(shard_diff))[0]
    print(f"Worst under-provisioning cases ({len(worst_cases)} samples):")
    for idx in worst_cases[:5]:  # Show first 5 worst cases
        actual_threads = y_test.iloc[idx]
        predicted_threads = y_pred[idx]
        actual_shard = actual_shards[idx]
        predicted_shard = predicted_shards[idx]
        print(f"  Sample {idx}: {actual_threads} threads ({actual_shard} shards) -> {predicted_threads} threads ({predicted_shard} shards)")

# Add shard analysis to the existing visualization
fig2, axes2 = plt.subplots(1, 2, figsize=(15, 6))

# Plot 1: Shard prediction accuracy over time
axes2[0].plot(y_test.index, actual_shards, label='Actual Shards', alpha=0.8, linewidth=2, marker='o', markersize=4)
axes2[0].plot(y_test.index, predicted_shards, label='Predicted Shards', alpha=0.8, linewidth=2, marker='s', markersize=4)
axes2[0].set_title('Shard Requirements: Actual vs Predicted')
axes2[0].set_xlabel('Timestamp')
axes2[0].set_ylabel('Required Shards')
axes2[0].legend()
axes2[0].grid(True, alpha=0.3)

# Plot 2: Shard difference distribution
axes2[1].hist(shard_diff, bins=np.arange(-4.5, 5.5, 1), alpha=0.7, color='lightblue', edgecolor='black')
axes2[1].axvline(x=0, color='red', linestyle='--', linewidth=2, label='Perfect Match')
axes2[1].axvline(x=np.mean(shard_diff), color='green', linestyle='-', linewidth=2, 
                label=f'Mean Diff: {np.mean(shard_diff):.2f}')
axes2[1].set_title('Shard Prediction Difference Distribution')
axes2[1].set_xlabel('Shard Difference (Predicted - Actual)')
...
# Summary of shard prediction performance
print(f"\n--- Shard Prediction Summary ---")
print(f"✓ Overall shard accuracy: {shard_accuracy*100:.1f}%")
print(f"✓ Over-provisioning (safer): {shard_over_provision/len(actual_shards)*100:.1f}%")
print(f"✓ Under-provisioning (risky): {shard_under_provision/len(actual_shards)*100:.1f}%")
print(f"✓ Average shard bias: {np.mean(shard_diff):.2f} shards")

if np.mean(shard_diff) > 0:
    print("✓ Model tends to over-provision shards (GOOD for avoiding resource shortages)")
elif np.mean(shard_diff) < 0:
    print("⚠ Model tends to under-provision shards (RISKY - may cause performance issues)")
else:
    print("✓ Model is well-balanced in shard provisioning")